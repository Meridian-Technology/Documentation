...
| 1.7 | done | doneAt 2026-08-24; physical-phone saved-process, worker ownership, restart recovery, and rollback acceptance passed |
| 1.8 | done | doneAt 2026-08-24; child #12 audited, parent ledger updated, and adjacent scope excluded |
...
- [x] James explicitly authorized real Mini acceptance
- [x] Web, Just Go, and Campus run one at a time and stop cleanly
- [x] Worker block and run-owned preview behavior match the contract
- [x] Restart leaves no duplicate/stale owned child
- [x] No private value or secret entered Git history

**As built:**

| Item | Current state |
| --- | --- |
| Authorization | James explicitly authorized Task 1.7 on 2026-08-24 after pushing the reviewed Relay commits and running the Mini deploy command. |
| Mini preflight | Confirmed the reviewed Relay commit on a clean checkout, a healthy LaunchAgent, mode-`0600` private configuration and backup, bearer rejection/success (`401`/`200`), healthy memory pressure with no swap, and an idle/unblocked machine. Installed the three private saved profiles and verified the sanitized API catalog without recording their private values. |
| Phone baseline | The physical phone reached the Mini over Tailscale. Processes showed the exact three profiles idle with their expected ports and URL kinds, and Workspace showed read-only Git state for all configured repositories. |
| Compatibility repair | Physical acceptance exposed that the Mini's older Meridian CLI printed human status for `status --json --local`. Added the scoped local-JSON CLI commit, installed it on the Mini, verified a valid `localOnly: true` snapshot for Meridian and Events without a fetch, and confirmed the refreshed phone Workspace cleared both partial-status warnings. The first Web start then exposed raw `os.freemem()` misclassifying healthy macOS reclaimable memory as critical despite 72% system availability and no swap; the deployed scoped Relay hotfix now uses `memory_pressure -Q` on macOS with a safe raw-memory fallback. |
| Real Web result | Meridian Web starts, opens, and exposes logs successfully from the physical phone after the memory hotfix. Its missing Mongo dependency is an expected application-environment limitation for this Mini and not a Relay launch/control failure. Stop returned ownership to idle and released port 3000 while ports 8081 and 8082 remained free. At James's request, the phone now has a dedicated read-only active-process log route with two-second byte-offset polling, full in-view session retention, ANSI cleanup, and basic error/warning/success/timestamp highlighting. The output is edge-to-edge beneath a translucent blurred native header, stays pinned to the newest output, uses an icon-only refresh action, and exposes no command input. |
| Real Just Go result | Just Go starts from the physical phone, exposes live Metro logs in the dedicated viewer, and its returned development-client link opens successfully. Only port 8081 listened, macOS retained 83% system-wide available memory, and Stop returned ownership to idle with all three managed ports free. |
| Real Campus result | James independently verified the same isolated phone start/open/log/stop and port-release lifecycle for Campus, with no concurrent managed listener left behind. |
| Run-control repair | The pre-worker audit found that Processes linked to Run for preview controls but the phone Run screen only displayed/opened the current preview. Added explicit active-run Off/Expo/Web controls over the existing authenticated stable-enum endpoint, preserving the selected machine origin and using only the server-returned URL for Open. |
| Worker ownership | Backed up the private environment with mode `0600`, selected the fake adapter without printing its other values, and started a harmless run parked at an `approve-before` gate. Processes reported run ownership and a worker block with every manual Start disabled, while Workspace remained authenticated, read-only, and available for all five repositories. The phone opened the active Run from Processes and switched its owned preview from Web to Expo using the new stable-enum controls. |
| Restart recovery | Before restart, Expo had one listener and the parked run owned the supervisor. After a LaunchAgent restart, Relay recovered the same run ID and worker block. The API returned before Metro finished binding; readiness polling then proved exactly one Expo listener, no listeners on the Web/Campus ports, and no duplicate or stale child. A replacement PID was accepted as expected restart behavior. |
| Fake completion | The phone selected Off before completing the gate, the harmless fake-adapter step completed, Processes returned to idle, and all manual Start controls became available again. No vendor adapter or real repository edit ran. |
| Rollback evidence | The exact original private environment was restored byte-for-byte at mode `0600`; its timestamped backup and the private acceptance record remain preserved off-Git. All five repositories were clean, their original branches were restored, the temporary acceptance branch had no unique commits and was removed, Relay restarted successfully, ownership was idle/unblocked, and ports 3000, 8081, and 8082 were free. |
| Operator docs and privacy | Expanded the Mini runbook with placeholder-only saved-process, worker-block, restart-readiness, and rollback procedures, including the delayed-listener check and interactive-SSH caution. No bearer, credential, private environment value, filesystem path, IP, or new machine hostname was added to Git during Task 1.7. |
| Follow-up and stop | Task 1.7 is complete. Task 1.8 is the separate child-closeout audit; no push, arbitrary-process, backend-profile, web-UI, or Git-mutation surface begins here. |
...
- [x] Parent child #12 row is current
- [x] No adjacent/unapproved surface entered closeout

**As built:**

- Audited Tasks 0.1–1.7 against the child ledger, task statuses, evaluation checkboxes, As built tables, handoff entries, offline boundaries, and the completed physical-Mini acceptance/rollback evidence. Every prior task is aligned and no implementation note remains open.
- Re-ran the current implementation verification: all 276 Relay tests passed with one worker and the Relay TypeScript build passed; all 71 phone tests passed and phone typecheck passed. Closeout consistency, diff, and changed-file boundary checks also passed.
- Marked parent child #12 done with `doneAt 2026-08-24` and the canonical `/strategy/relay-workspace-process-plan` Mintlify path in both the child-plan ledger and Related docs.
- This closeout changed documentation only. It did not begin push/Live Activities (#9), arbitrary process or shell controls, a backend profile, web Workspace/Processes UI, Git mutation actions, or any other unapproved adjacent surface.
...
- 2026-08-24 — Task 1.7 started with James's explicit authorization after the reviewed Relay system commits were pushed and the Mini deployment command was run. Paused at the first Mini checkpoint for a non-secret private-profile backup/preflight before any real saved process starts. No private value has been requested or recorded in Git. Next: run the provided Mini preflight and return only its sanitized summary.
- 2026-08-24 — Task 1.7 Mini preflight and phone baseline passed: reviewed Relay commit and clean checkout, healthy mode-restricted service/config/backup, bearer `401`/`200`, healthy memory pressure with no swap, sanitized three-profile catalog, physical-phone Tailscale access, idle Processes rows, and read-only Workspace Git state. The phone exposed an older-Meridian compatibility gap in `status --json --local`; a scoped CLI fix was committed, installed cleanly on the Mini, and verified as valid local-only JSON without fetching. No private hostname, IP, path, URL, environment value, or secret was recorded in Git. Next: refresh Workspace to clear the partial warning, then begin the one-at-a-time real process matrix.
- 2026-08-24 — Task 1.7 phone Workspace refresh passed with full Meridian alignment and no partial-status warnings. The first Meridian Web start remained idle because the memory gate used raw `os.freemem()` and rejected the Mini while macOS reported 72% system-wide availability and zero swap. Added scoped Relay commit `18baaf6` to use the authoritative macOS availability percentage with a bounded command timeout and safe raw-memory fallback; focused 9 tests, all 276 Relay tests, and the TypeScript build pass. No safety gate was bypassed and no real process started. Next: deploy the hotfix and retry the isolated Web start.
- 2026-08-24 — Task 1.7 Meridian Web launch and Open passed from the physical phone after deploying the macOS memory-gate hotfix; the visible Mongo failure is an expected missing app dependency and does not affect Relay process ownership. At James's request, added a dedicated typed Process Logs page reachable only for the active manual session, retaining bounded read-only byte-offset polling while adding ANSI cleanup and basic semantic line highlighting. Focused 9 tests, all 69 phone tests, and phone typecheck pass; no interactive terminal, command input, arbitrary process, or secret surface was added. Next: deploy the phone commit, capture Web logs, stop/release port 3000, then continue Just Go and Campus.
- 2026-08-24 — Refined the explicitly requested Process Logs page to retain the full session output while open, use larger terminal text across an edge-to-edge screen, scroll beneath a translucent native blur header, place process/live state in that header, and expose refresh as an icon-only top-right action. Focused 9 tests, all 69 phone tests, and typecheck pass; the API remains byte-offset/read-only and no interactive shell or command input was introduced.
- 2026-08-24 — Task 1.7 Meridian Web lifecycle passed: physical-phone start/open/log visibility, expected app-level missing-Mongo limitation, manual stop back to idle, and clean port 3000 release with ports 8081/8082 remaining free. Next: run the isolated Just Go lifecycle.
- 2026-08-24 — Task 1.7 saved-process matrix passed. Just Go ran alone on port 8081 with its development-client link and live logs working, retained 83% macOS available memory, then stopped with all managed ports free. James also verified the equivalent isolated Campus lifecycle. Web, Just Go, and Campus now satisfy the one-at-a-time start/open/log/stop/port-release criterion. Next: controlled fake-worker blocking, read-only Workspace visibility, and Run-owned preview switching.
- 2026-08-24 — Before the fake-worker checkpoint, found and closed a phone integration gap: Processes already handed run ownership to Run, but Run lacked controls for the existing run-preview switch API. Added active-run Off/Expo/Web controls with machine-scoped requests and server-returned Open only. Focused 28 tests, all 71 phone tests, and typecheck pass; no arbitrary preview, command, profile editing, or manual-process takeover surface was added. Next: deploy the phone commit and execute the controlled fake-worker matrix.
- 2026-08-24 — Task 1.7 complete. With James's authorization, the physical phone passed read-only Workspace and the isolated Meridian Web/Just Go/Campus start/open/log/stop matrix; the scoped Meridian JSON and macOS memory-gate repairs were deployed and verified. A private mode-`0600` backup protected the temporary fake-adapter change. A harmless `approve-before` run blocked every manual Start while Workspace stayed available, switched its owned preview Web-to-Expo from Run, and survived a Relay restart with the same run ID and exactly one listener after readiness polling. Off plus fake completion returned the machine to idle. Rollback restored the private environment byte-for-byte, original clean branches, no temporary branch, and all managed ports free; private backup/evidence stayed off-Git. Placeholder-only operator guidance now records the procedure. Next: Task 1.8 closeout audit; do not begin adjacent scope.
- 2026-08-24 — Task 1.8 complete. Audited Tasks 0.1–1.7, aligned the child ledger, statuses, evaluations, As built records, handoffs, offline boundaries, and physical-Mini rollback evidence, then re-ran all 276 Relay tests/build and all 71 phone tests/typecheck. Marked parent child #12 done with `doneAt 2026-08-24` and `/strategy/relay-workspace-process-plan`. This documentation-only closeout did not begin push/Live Activities (#9), arbitrary process or shell controls, a backend profile, web Workspace/Processes UI, Git mutation actions, or another adjacent child. Child #12 is closed.
