...
2. **Phase 2:** Embeddings (description + enriched fields) in a vector index; nightly batch jobs. See [Just Go — embeddings recommendations](/strategy/just-go-embeddings-recommendations) for data readiness, shadow evaluation, and go-live gates.
...
Organizer identity (city-scoped `PivotOrganizer`, crawl-time attach, backfill, Catalog panel) is a later workstream — do **not** create a campus Org per venue. Implementation: [Just Go organizer identity](/strategy/just-go-organizer-identity-plan).
