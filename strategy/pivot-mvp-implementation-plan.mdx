...
User-facing voice is **action-first**: wordmark **`just go*`**, CTAs like **“just go”** (referral continue, future swipe actions), and copy that frames the loop as *swipe → interested → get tickets → go*. Internal paths stay `pivot` / `AppEdition`; strings live in `Meridian-Mobile/src/pivot/copy/stoopCopy.ts` (filename legacy — exports `STOOP_COPY`).
...
| Week ticker | “swipe first. tickets later. just go*” |
