...
### v0 ranker (rules baseline)

`getPivotFeed` in `pivotFeedService.js` re-sorts the published weekly catalog:

```text
friend registered ↓
friend interested ↓
|event.tags ∩ user.pivotInterestTags| ↓
|event.tags ∩ tagsFromLowRatedEvents| ↑  (penalty)
start_time ↑
```

Same candidate set for everyone in the city-week; only **order** is personalized. This is the control system for any future model evaluation.
