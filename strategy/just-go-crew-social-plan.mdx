...
| 1.1 | pending | `PivotCrew` + `PivotCrewMembership` schemas |
| 1.2 | pending | Crew CRUD + invite link APIs |
| 1.3 | pending | Invited placeholder members |
| 1.4 | pending | Onboarding: start a crew step |
| 1.5 | pending | `PIVOT_COPY.crew` + migrate cohort step copy |
| 1.6 | pending | Profile: crews section (secondary to you) |
| 2.1 | pending | `PivotCrewWeekState` aggregation job |
| 2.2 | pending | `GET /pivot/crews/week` progress + proposed picks |
| 2.3 | pending | Group judgement APIs (confirm / swap) |
| 2.4 | pending | Week UI: post-deck crew progress + judgement sheet |
| 2.5 | pending | Nudges: unfinished swipes + solo crew CTA |
| 2.6 | pending | Recap hero: crew pick on top of personal list |
...
- [ ] Schemas registered in getModels
- [ ] Migrations/seed script for dev tenants
- [ ] User can belong to multiple crews
...
- [ ] Create/list/join flows covered by route outcome tests
- [ ] Invite link opens app to crew join screen
- [ ] Owner can invite; members can invite (configurable later — default **all members**)
...
- [ ] Invited rows visible on roster before signup
- [ ] Invited never counted in quorum (server-enforced)
- [ ] Generic label only in API + UI
...
- [ ] Onboarding completes without crew (solo path)
- [ ] Creating crew during onboarding lands user on Week with crew roster visible
- [ ] Skip does not block deck
...
- [ ] All new UI strings in `pivotCopy.ts` only
- [ ] Voice matches existing Just Go patterns
...
- [ ] Unlimited crews list without perf issues (paginate if >20)
- [ ] Tap crew → roster + invite CTA
...
- [ ] Aggregation matches hand-calculated fixtures in unit tests
- [ ] Invited excluded from quorum
- [ ] Split votes produce top-2 candidates, not auto-resolution
...
- [ ] Week screen can poll progress after each swipe
- [ ] Response cached ≤30s or invalidated on intent write
...
- [ ] Confirm after window → 409 with clear error
- [ ] Swap only among top candidates for split resolution
- [ ] Locked pick visible in recap API
...
- [ ] Solo user sees no judgement sheet
- [ ] Invited shown as “invited” in breakdown
- [ ] Analytics: `pivot_crew_judgement_confirm`, `_swap`
...
- [ ] Nudge respects config thresholds
- [ ] Solo path identical to current MVP deck behaviour
...
- [ ] Personal interested events still fully listed
- [ ] Crew pick visually primary when present
- [ ] Multiple crews → stacked hero cards (user resolves each)
