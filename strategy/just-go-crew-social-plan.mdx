...
<Note>
**Doc ownership:** This plan owns judgement **mechanics** (quorum, propose, confirm/swap, consensus timer, split). Mobile Week **ritual shell** (APIs, deck, recap, store cutover) is in [Just Go — crew Week experience (mobile)](/strategy/just-go-crew-mobile-experience-plan). Primary decide **surfaces** (Week hub + per-crew room) are in [Just Go — decide UI rewrite](/strategy/just-go-decide-ui-rewrite-plan) — that plan revises the experience plan’s Phase 9 pager decision for decide UX only.
</Note>
...
| 4 | Group outcome | **Swipe shortlist (top 3)** → **Borda rank** by actives → **tally auto-locks** (no confirm / swap) |
...
open app (no invite code gate)
  ...
  → quorum shortlists top 3 → crew ranks (Borda) → tally locks
...
Partiful/Luma remain ticket surfaces. Meridian remains discovery + intent + **group pick**.
...
| **`PivotCrewWeekState`** | Per crew × `batchWeek`: swipe progress, shortlist, ballots, locked pick |
| **`PivotCrewJudgement`** | Borda ballot + tally lock (no confirm / swap) |
| **`pivotCrewConfig`** | Tenant-scoped tunables (weights, quorum, ballot deadline) |
...
2. For each crew, when quorum hits, freeze a **top-3 shortlist** from member swipes (active members only).
3. Each active **ranks the shortlist once** (Borda). Tally locks when everyone has balloted or the ballot deadline hits.
4. **Personal recap** lists all personal `interested` events; **hero card** shows locked crew pick(s) when applicable.
...
| Step | Behaviour |
| --- | --- |
| **Shortlist** | Server freezes top **1–3** events from weighted-majority swipe scores (`voteBreakdown`) |
| **Balloting** | Each **active** member ranks the shortlist once (partial ranks OK). Status = `balloting`. |
| **Borda tally** | Points = `n − rankIndex − 1` (for 3: 2 / 1 / 0). Tie-break: most #1s, then `most_registered_then_earliest_start`. |
| **Lock** | When **all actives have balloted**, or **ballot / hard deadline** hits (non-voters abstain). Winner → `confirmed`. Single-candidate shortlist skips ballot and locks immediately. |
| **No confirm / swap** | Ranking is the only Decide action. No shared swap budget or consensus confirm timer. |

Invited members appear as **“invited”** on roster and vote breakdown but **never** affect quorum or ballots.

Mobile decide **surfaces** (Week hub triage + per-crew room) are tracked in [Just Go — decide UI rewrite](/strategy/just-go-decide-ui-rewrite-plan). Ritual phase / Week shell remain in [Just Go — crew Week experience (mobile)](/strategy/just-go-crew-mobile-experience-plan). This section remains the source of truth for judgement mechanics (Borda auto-lock).
...
| `judgement.ballotWindowMinutes` | 180 | Soft ballot clock after quorum (capped by hard window) |
| `judgement.consensusWindowMinutes` | 180 | Deprecated (confirm/swap); kept for config compat |
| `judgement.swapResetBonusMinutes` | 15 | Deprecated (confirm/swap); kept for config compat |
| `judgement.crewSwapBudget` | 2 | Deprecated (confirm/swap); kept for config compat |
| `judgement.maxPickSlots` | 1 | Tenant default pick capacity (1–2); v1 Borda locks a single winner |
| `pick.algorithm` | `weighted_majority` | Builds top-3 shortlist (`registered` > `interested`) |
| `pick.ballotMethod` | `borda` | Decide tally over shortlist |
...
| 3.1 | done | Ranker: crew signal boosts from config weights; doneAt: 2026-07-21 |
| 3.2 | done | Ranker: interest bleed from crew members' tags; doneAt: 2026-07-21 |
| 3.3 | done | Deck card copy: crew social proof; doneAt: 2026-07-21 |
| 3.4 | done | Log crew context on `PivotInteraction`; doneAt: 2026-07-21 |
| 4.1 | done | Cross-crew overlap detection; doneAt: 2026-07-21 |
| 4.2 | done | Surface on recap + event detail; doneAt: 2026-07-21 |
| 4.3 | done | Explore crews going rail; doneAt: 2026-07-21 |
| 5.1 | done | Contacts permission + hash match pipeline; doneAt: 2026-07-21 |
| 5.2 | done | Onboarding contacts accelerator UI; doneAt: 2026-07-21 |
| 6.1 | done | Platform Admin crew config editor; doneAt: 2026-07-21 |
| 6.2 | done | Tenant dashboard crew funnel metrics; doneAt: 2026-07-21 |
| 6.3 | done | Push copy: “stop planning” crew ritual; doneAt: 2026-07-21 |
| 6.4 | done | Ritual dry-run script + runbook section; doneAt: 2026-07-22 |
...
| Method | Path | Role |
| --- | --- | --- |
| `POST` | `/pivot/crews/:crewId/week/confirm` | Lock pick set (`eventId`, optional `eventIds[]`) |
| `POST` | `/pivot/crews/:crewId/week/swap` | Replace a slot (`eventId?`, `slotIndex?`; omit event → runner-up) |
| `GET` | `/pivot/crews/:crewId/week/judgement` | `proposedEvents[]`, `maxPickSlots`, breakdown |
| `PATCH` | `/pivot/crews/:crewId/settings` | Owner sets `maxPickSlots` (1 \| 2 \| null inherit) |
...
- [x] With config weights = 0, behaviour matches pre-change ranker
- [x] Unit tests for crew boost ordering
- [x] `rankerVersion` remains `rules_v0` until embeddings ship
...
- [x] Bleed never exceeds `maxWeight` impact on sort key
- [x] Disabled via config → zero bleed
...
- [x] Counts accurate for multi-crew overlap (dedupe members)
- [x] Zero crews → fields omitted
...
- [x] Deck impressions include crew context when user has crews
- [x] No breaking change to existing log consumers
...
- [x] No surface when `crossCrew.enabled = false`
- [x] User in only one crew still sees overlap when friend graph connects
...
- [x] Surface disappears if overlap crew unconfirms
- [x] Analytics: `pivot_cross_crew_surface_view`
...
- [x] Rail hidden when user has zero crews
- [x] Same week catalog scope as existing explore
...
- [x] Permission denial does not block onboarding
- [x] Privacy review note in PR
- [x] Hashes only in transit + match table
...
- [x] Copy explains we don’t message contacts
- [x] Skip rate tracked in analytics
...
- [x] Changes reflect in `GET /pivot/config` within 60s
...
- [x] Overview tab widgets in Pivot tenant dashboard
- [x] Filters by `batchWeek`
...
- [x] Push still opens Week tab only
- [x] Solo users get existing drop copy
...
**Script:** Automated matrix via `npm run dry-run:pivot-crew-ritual` (solo, 1 / 3 crews, split, invited-only) plus manual staging checklist in [weekly drop runbook](/strategy/pivot-weekly-drop-runbook#crew-ritual-dry-run-task-64--mobile-phase-12).

**Evaluation criteria:**

- [x] Runbook section added to [weekly drop runbook](/strategy/pivot-weekly-drop-runbook)
- [x] All Phase 0–4 tasks `done` or explicitly waived
...
- [Just Go — crew Week experience (mobile)](/strategy/just-go-crew-mobile-experience-plan) — ritual shell, deck, recap, cutover
- [Just Go — decide UI rewrite](/strategy/just-go-decide-ui-rewrite-plan) — primary decide surfaces (hub + room)
