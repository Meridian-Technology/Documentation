...
- [Just Go organizer identity](/strategy/just-go-organizer-identity-plan) — Catalog page (`?page=4`) on this tenant shell; not a Curation tab
...
A **Pivot Tenant Ops Dashboard** — one Dashboard shell per pivot city (or one shell with tenant switcher locked to pivot tenants), entered from Tenant Management. Primary surfaces (later pages were **appended** so `?page=0..2` bookmarks stay stable):
...
4. **Drop deck** (`?page=3`) — city deck config.
5. **Catalog** (`?page=4`) — weekless organizer graph (backfill, merge leftovers, claim). Not a Curation tab and not fleet Lab Catalog. See [Just Go organizer identity](/strategy/just-go-organizer-identity-plan).
...
| Routes | **Locked:** `/platform-admin/pivot/:tenantKey` + Dashboard `?page=` (`0` Overview, `1` Curation, `2` User journeys, `3` Drop deck, `4` Catalog) — see [Task 0.2](#task-02--information-architecture-and-route-map) |
...
| `3` | **Drop deck** | City deck config (appended — do not insert earlier) | Tenant drop / deck tools |
| `4` | **Catalog** | Weekless organizer graph: list, dossier, unlinked leftovers, backfill, merge. Claim is ops-granted API. **Not** a Curation tab. | [Just Go organizer identity](/strategy/just-go-organizer-identity-plan) |
...
| **Shell** | Own `Dashboard` instance: `menuItems` = Overview, Curation, User journeys, Drop deck, Catalog (3 and 4 **appended**); `middleItem` = pivot tenant switcher; `onBack` → `/platform-admin?page=0` (Tenants) |
| **Page index** | Reuse Dashboard’s existing `?page=` query (`0` Overview, `1` Curation, `2` User journeys, `3` Drop deck, `4` Catalog) |
...
| Catalog (organizer graph) | `/platform-admin/pivot/:tenantKey?page=4` |
| Catalog unlinked / ambiguous | `/platform-admin/pivot/:tenantKey?page=4&filter=unlinked` or `filter=ambiguous` |
| Catalog dossier | `/platform-admin/pivot/:tenantKey?page=4&organizerId=` |
