...
When [multi-tenant identity](/backend/multi-tenant-identity) is enabled, sessions are stored in the **global database** and keyed by `globalUserId`, so token refresh works across all tenant subdomains (SSO). Otherwise, sessions may be stored per-tenant. The refresh flow validates the session and resolves the tenant user for the current `req.school` when issuing a new access token.
...
3. Access token (short-lived; default **15 minutes** unless overridden by env) and refresh token (default **30 days**) are set as HTTP-only cookies
...
<CardGroup cols={2}>
  <Card title="Authentication overview" icon="shield" href="/backend/auth/overview">
    Email, Google, Apple, JWT cookies vs mobile headers, and how login reaches `issueTokens`.
  </Card>
  <Card title="SAML" icon="key" href="/backend/auth/SAML">
    University SSO; after callback, tokens and sessions align with the same global model.
  </Card>
  <Card title="Multi-tenant identity & SSO" icon="users" href="/backend/multi-tenant-identity">
    Why sessions live in the global DB and how refresh works across subdomains.
  </Card>
  <Card title="Backend best practices" icon="server" href="/backend/best-practices">
    Middleware, `getGlobalModelService`, and patterns next to session-aware routes.
  </Card>
</CardGroup>
