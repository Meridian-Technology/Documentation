...
Registrations are attributed to the source that brought the user to the event page. For each `event_registration`, the backend finds the most recent `event_view` before the registration from the same user (anonymous_id or user_id) and uses that view's source.
