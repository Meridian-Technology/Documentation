...
## Event views and referrer sources

Event organizers can see **how many people viewed an event** and **where those views came from** (e.g. direct, explore, org page, QR codes, **email**). This is powered by the `event_view` event and optional `properties.source`.

### `event_view`

When a user lands on an event page, the client sends an `event_view` event with at least:

- `properties.event_id` — the event being viewed.

Optional referrer attribution:

- `properties.source` — one of `direct`, `explore`, `org_page`, `qr`, or **`email`**.
- For QR: `properties.qr_id` — the QR short id that was scanned.

If `source` is omitted, the backend infers it from `context.referrer` (e.g. URLs containing `org/` or `club-dashboard` → `org_page`, `events-dashboard` → `explore`, otherwise `direct`).

### Views from announcement emails

When organizers send **event announcement emails**, the “View Event” and event-name links in the email include `?source=email`. When a recipient clicks through, the event page sends `event_view` with `properties.source: 'email'`. The event analytics API aggregates these and exposes them as **Sources → Email** in the Event Dashboard **Analytics** tab, so organizers can see how many people viewed the event after clicking from an announcement email.
...
<CardGroup cols={2}>
  <Card title="Analytics collected events" icon="list" href="/backend/analytics-collected-events">
    Canonical list of event names and properties emitted by clients.
  </Card>
  <Card title="Referrer sources" icon="share-nodes" href="/backend/analytics-referrer-sources">
    URL-to-source mapping used in aggregations and dashboards.
  </Card>
  <Card title="Mobile analytics" icon="smartphone" href="/mobile/analytics">
    Mobile client queue, batching, and alignment with the envelope schema.
  </Card>
  <Card title="Atlas event analytics" icon="chart-column" href="/atlas/event-analytics">
    Organizer-facing charts and funnels backed by this pipeline.
  </Card>
  <Card title="Event dashboard" icon="table-columns" href="/atlas/event-dashboard">
    Where event views and email-driven sources appear for organizers.
  </Card>
  <Card title="Backend best practices" icon="server" href="/backend/best-practices">
    Where ingestion routes live alongside the rest of `Meridian/backend`.
  </Card>
</CardGroup>
