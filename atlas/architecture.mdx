...
- **Backend surfaces**:
  ...
  - **Org budgets (Phase 2 finance MVP)**: `Meridian/backend/routes/orgBudgetRoutes.js` (mounted at `/org-budgets`)
...
**Atlas CMS Phase 1 (additional routes):**

- `PATCH /org-management/organizations/:orgId/lifecycle` — platform admin lifecycle transitions (validated against `atlasPolicy`).
- `PUT /org-management/organizations/:orgId/governance/:docKey/versions/:version/approve` — approve a governance PDF version.
- `PATCH /org-roles/:orgId/lifecycle` — org leaders (`manage_roles` or `manage_settings`) may transition per policy.
- `GET|POST /org-roles/:orgId/governance/...` — list requirements, upload PDF (`manage_governance`).
- `GET /org-roles/:orgId/members/:userId/history` — membership and removal audit.
