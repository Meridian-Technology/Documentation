...
- **Returns**: `{ data, loading, error, errorCode, errorStatus, refetch }`. Call **`refetch()`** after a successful mutation to align UI with the server.
- **Branching on a specific failure**: `error` is a human-readable message — do **not** parse it. Use **`errorCode`** (the backend's stable `code` from the error body) and **`errorStatus`** (HTTP status) when a surface must render a distinct state for one failure, e.g. `errorStatus === 403 && errorCode === 'CREATOR_FORBIDDEN'` rendering an access gate instead of a generic error panel.
...
- **Return value**: on HTTP success, returns **`response.data`** (the server JSON). On failure, returns **`{ error, code, errorCode? }`** objects rather than throwing — **check the shape** in the caller (`if (result.error) …`).
  - **`code`** is the **HTTP status** (or a transport label such as `NETWORK_ERROR`).
  - **`errorCode`** is the backend's stable **body `code`**, present only when the error response carried one. Branch on `errorCode` — never on the message text — when a specific failure needs its own handling, e.g. mapping a validation code onto the form field it belongs to.
...
if (res?.error) {
  // res.error = message, res.code = HTTP status, res.errorCode = backend body code
  return;
}
